...
// This file has been modified by Vincent Rebiscoul <vincent.rebiscoul@ens-lyon.fr>
...
expression
  : expression unop=(INC | DEC)				#postIncOrDecExpression
  | 'new' typeName					#newTypeNameExpression
  ...
  | unop=(INC | DEC) expression				#preIncOrDecExpression
  | unop=(PLUS | MINUS) expression			#unaryAdditiveExpression
  | ('after' | 'delete') expression			#deleteExpression
  | '!' expression					#notExpression
  | '~' expression					#tildeExpression
  | expression '**' expression				#powerExpression
  | expression binop=(MULT | DIV | MOD) expression	#multiplicativeExpression
  | expression binop=(PLUS | MINUS) expression		#additiveExpression
  | expression binop=(LSL | LSR) expression		#logicalShiftExpression
  ...
  | expression binop=('=' | '|=' | '^=' | '&=' | '<<=' | '>>=' | '+=' | '-=' | '*=' | '/=' | '%=') expression	#assignmentExpression
  | primaryExpression					#primaryExprExpression
;
